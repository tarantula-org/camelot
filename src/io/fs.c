...
// --- INTERNAL IMPLEMENTATION (Keep stream/slurp as is) ---
static u64 internal_stream(File *f, Op op, void *arg, u64 num) {
      ...
      switch (op) {
            case OPEN: {
                  ...
                  if (!h) { f->status = FILE_NOT_FOUND; f->size = 0; return 0; }
                  if (fseek(h, 0, SEEK_END) == 0) {
                        long len = ftell(h);
                        fseek(h, 0, SEEK_SET);
                        f->size = (u64)len;
                  } else { clearerr(h); f->size = 0; }
                  f->handle = h; f->status = OK;
                  return 1;
            }
            case READ: {
            ...
            case CLOSE: {
                  if (f->handle) { fclose((FILE*)f->handle); f->handle = NULL; f->status = IO_ERROR; }
                  return 0;
            }
      }
      return 0;
...
static String internal_slurp(Arena *a, const char *path) {
      ...
      if (f.size == 0) { internal_stream(&f, CLOSE, NULL, 0); return (String){0}; }

      u8 *buf = arena.alloc(a, f.size + 1);
      if (!buf) { internal_stream(&f, CLOSE, NULL, 0); return (String){0}; }
