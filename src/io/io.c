...
String scan(Arena *a, u64 cap) {
      if (cap == 0) cap = IO_DEFAULT_SCAN_CAP;
...
void print(const char *fmt, ...) {
      ...
      for (const char *p = fmt; *p != '\0'; p++) {
            ...
            p++; 
            switch (*p) {
                  case 'i': put_i64(va_arg(args, int)); break;
                  case 'f': put_f64(va_arg(args, double)); break;
                  case 's': { 
                        char *c = va_arg(args, char*);
                        write(1, c, strlen(c));
                        break;
                  }
                  case 'S': put(va_arg(args, String)); break;
                  
                  case '%': write(1, "%", 1); break;

                  default:  write(1, p-1, 1); p--; 
            }
      }
      va_end(args);
}